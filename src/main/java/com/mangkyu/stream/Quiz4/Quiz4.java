...
public class Quiz4 {
    ...
    public List<Transaction> quiz1() {
        return transactions.stream()
                .filter(transaction -> transaction.getYear() == 2020) // 2020년도 조건
                .sorted(Comparator.comparing(Transaction::getValue)) // sorted Transaction 거래값 비교, 내림차순은 reversed() 뒤에 붙이기
    ...
    public List<String> quiz2() {
        return transactions.stream()
                .map(transaction -> transaction.getTrader().getCity()) // 거래 내역의 거래자의 도시로 변환
                .distinct() // 중복 없이 처리
                .collect(Collectors.toList()); // 리스트화
    }
    ...
    public List<Trader> quiz3() {
        return transactions.stream()
                .map(transaction -> transaction.getTrader()) // 거래내역의 거래자
                .distinct() // 중복이면 제거
                .filter(trader -> trader.getCity().equals("Seoul")) // 서울에서 거래했다면
                .sorted(Comparator.comparing(Trader::getName)) // 거래자 이름 순서대로(오름차순)
                .collect(Collectors.toList()); // 리스트화
    }
    ...
    public String quiz4() {
        return transactions.stream()
                .map(transaction -> transaction.getTrader().getName()) // 거래 내역의 거래자 이름 추출
                .distinct() // 중복 제거
                .sorted() //  String 스트림 오름차순 정렬
                .collect(Collectors.joining(",")); // 구분자 포함해서 String 값으로 만들기
    }
    ...
    public List<Integer> quiz6() {
        return transactions.stream()
                .filter(transaction -> transaction.getTrader().getCity().equals("Seoul")) // 거래 내역의 거래자의 도시가 Seoul 인지 확인
                .map(Transaction::getValue) // 일치하면 해당 거래 내역의 거래 금액 변경
                .collect(Collectors.toList()); // 모든 거래 금액 리스트화
    }
    ...
    public Integer[] quiz7() {
        ...
        array[0] = transactions.stream()
                ...
                .reduce(Integer::max) // 거래 금액 중 최대 값을 가져옵니다.
...
//                .orElseThrow(RuntimeException::new).getValue();
        ...
        array[1] = transactions.stream()
                .min(Comparator.comparingInt(Transaction::getValue)) // 거래 내역 스트림에서 거래 금액을 비교해 최솟값을 가져옵니다.
                .orElseThrow(RuntimeException::new).getValue(); // 존재하지 않으면 예외를 던진다.
